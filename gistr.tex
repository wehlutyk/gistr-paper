...
% FIXME: this seems broken on the cas-dc document class -- do we need numbered quotes?
...
platform.
% and many smaller pilots in between to test lessons learned in the larger runs
% and adjust task parameters and source complexity.
% The overall quality of the
% data we gathered gradually increased with the pilots.
...
% We start with the architecture common to all experiments.
...
hypothesise that
% the low-level cognitive mechanisms underlying utterance
...
%% The spam level of Experiment 1, and especially the
...
%% quality through the interface.
%% Nonetheless, it became clear that using
...
%We first provide an overview of the data and a few important surface trends related to the global conservation and loss of content, before proposing a more detailed model at the level of transformations.

\begin{figure}
  \centering
  \subfloat[Utterance length]{
    \includegraphics[width=.3\linewidth]{images/computed/length_loss-all_exps.png}
    \label{fig:length-loss}
  }
  \subfloat[Transmissibility]{
    \includegraphics[width=.3\linewidth]{images/computed/transmissibility-all_exps.png}
    \label{fig:transmissibility}
  }
  \subfloat[Variability]{
    \includegraphics[width=.39\linewidth]{images/computed/variability-all_exps.png}
...
%  at \url{http://spacy.io/}.}
%The insets show the data restricted to
...
% very long utterances (above 100
...
%until the end of the branches of Experiment 3.
...
appear.
%As replacements were the only process that could be extracted
...
%By abstracting out the basic building blocks of transformations, we will
...
%%. We do so in three stages. First, we
...
%therefore use it in both the utterance and the branch dimensions.
In the utterance dimension, this measure tells us to which extent changes
tend to occur in contiguous chunks.
%We will see that this chunkiness is very strong, so much that we will start
%examining insertions and deletions and the level of chunks.
